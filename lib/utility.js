...
exports.parseWinUserWords = parseWinUserWords;
exports.addWinUserWord = addWinUserWord;
...
const trueApp = process.type === 'renderer' ? _electron.remote.app : _electron.app;
...
function getCacheDirPath() {
  return _path2.default.join(trueApp.getPath('userData'), 'dictionaries');
}
...
function getWinUserWordsPath() {
  return _path2.default.join(getCacheDirPath(), 'winUserWords.json');
}
 ...
 */
function parseWinUserWords() {
  if (!_promisify.fs.existsSync(getWinUserWordsPath())) writeWinUserWords({});
  return JSON.parse(_promisify.fs.readFileSync(getWinUserWordsPath()));
}
...
function writeWinUserWords(wordMap) {
  _promisify.fs.writeFileSync(getWinUserWordsPath(), JSON.stringify(wordMap));
}